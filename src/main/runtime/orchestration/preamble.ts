...
=== CLI COMMANDS ===
  ...
  #
  # RULE: only this owning top-level turn may send worker_done, heartbeat,
  # escalation, or decision_gate. If you delegate to an internal subagent or
  # reviewer (cold review, code review, any spawned helper), it must hand its
  # conclusion back to you as review findings and must never run these
  # lifecycle commands itself. Orca authenticates the pane, not the caller
  # inside it, so a subagent's worker_done settles YOUR dispatch, revokes the
  # capability, and locks you out of reporting your real result.
  #
